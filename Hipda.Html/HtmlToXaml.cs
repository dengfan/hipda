...
namespace Hipda.Html
{
    ...
    {
        public static string Convert(string htmlContent, int maxImageCount, ref int imageCount)
        {
                ...
                if (matchsForImage1 != null && matchsForImage1.Count > 0)
                {
                    for (int i = 0; i < matchsForImage1.Count; i++)
                    {
                        ...
                        if (imageCount <= maxImageCount)
                        {
                            if (width >= 280)
                            {
                                imgXaml = @"↵[InlineUIContainer][Image Stretch=""Uniform"" Margin=""0,10,0,5"" MaxWidth=""500""][Image.Source][BitmapImage UriSource=""{0}"" /][/Image.Source][/Image][/InlineUIContainer]";
                            }
